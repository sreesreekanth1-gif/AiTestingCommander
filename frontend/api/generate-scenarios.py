...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        payload_path = "/tmp/ts_payload.json"
        with open(payload_path, "w", encoding="utf-8") as f:
            json.dump(payload, f)

        try:
            engine = TestScenariosEngine(payload)
            ...
            response = {
                "status": "success",
                "message": "Test scenarios generated.",
                "scenarios": scenarios_result.get("scenarios", []),
                "testPlanTitle": scenarios_result.get("testPlanTitle", "Test Scenarios")
            }
